...
get_latest_version() {
    ...
    API_RESPONSE=$(curl -sS "https://api.github.com/repos/aidan-bailey/loom/releases")
    if [ $? -ne 0 ]; then
...
extract_and_install() {
    ...
    mv "${tmp_dir}/loom${extension}" "$bin_dir/$INSTALL_NAME${extension}"
    rm -rf "$tmp_dir"
...
main() {
    ...
    RELEASE_URL="https://github.com/aidan-bailey/loom/releases/download/v${VERSION}"
    ARCHIVE_NAME="loom_${VERSION}_${PLATFORM}_${ARCHITECTURE}${ARCHIVE_EXT}"
