...
| `]` / `[` | Jump to next/prev agent waiting for input (Prompting or bell; **fleet-wide** — in focus mode crosses workspaces, switching the focused tab; in overview it moves only the cursor, no focus switch or promotion; wraps) |
...
| `enter` | (overview) Focus the selected card's workspace + instance (crosses workspaces; promotes a background workspace to a tab) |
...
**Cross-workspace overview (mission-control phase 4).** The overview is a **live global fleet view across every registered workspace**, loaded lazily and progressively. On the first `tab` into overview, `ensureFleetLoaded` fires background `tea.Cmd`s that reconcile each not-yet-open workspace off the Update goroutine (`loadWorkspaceForFleet`) and hand it back via `workspaceActivatedMsg` → `handleWorkspaceActivated`, which appends it as a **background slot** (`workspaceSlot.background`: fully live/reconciled/monitored, but hidden from the tab bar and never persisted to `OpenWorkspaces` — so next launch stays lazy; workspace-terminal auto-spawn is suppressed for these). Each workspace renders as its own card group (focused first, then alphabetical) with per-group states loaded/loading/error/empty. The overview carries a **global domain cursor** `home.overviewCursor {slot,inst}` (translated to render coords `ui.OverviewCursor {Group,Item}` in `overviewData`); `j/k` walk `fleetOrder` across all groups. Cursor-committing keys route through **`focusCursorSlot()`** (save current slot → promote the cursor's slot out of background → `loadSlot` → select), so `enter`/`D`/`r`/`n` reuse the existing focus-mode intents on the right workspace (and `D`/`r`/`n` therefore *promote* a background slot). Focus-mode `]`/`[` (`jumpWaiting`) is fleet-wide too, switching the focused slot as needed. Closing a tab while the fleet is engaged **demotes to background** (agents stay live) rather than tearing down (`applyWorkspaceToggle` fleet branch). **Invariant: the focused slot is never background.** Known v1 limitation: selecting a still-*loading* workspace in the picker drops to background (the fleet-toggle branch only sees materialized slots).
...
- **Fleet overview: background slots and the never-background-focused invariant.** `workspaceSlot.background` marks a slot loaded only to feed the live global overview (Cross-workspace overview above). Two rules bite: (1) **the focused slot is never background** — `demoteSlot` refuses the focused slot, and every focus-switch (`focusCursorSlot`, `jumpWaiting` landing, `applyWorkspaceToggle` fleet branch) promotes the target before/at load; a broken sequence that leaves the focused slot background silently corrupts `foregroundSlotsAndSelected`'s tab-bar index. (2) **background slots must stay out of `OpenWorkspaces`** — tab bar and persistence go through `foregroundSlotNames()`/`foregroundSlotsAndSelected()`, never `slotNames()`, and the workspace picker seeds its pre-checked set from `pickerActiveNames()` (foreground only); seeding it from all slots re-promotes the whole fleet on a bare picker open/close and breaks lazy loading (regression: `TestPickerActiveNames_ExcludesBackground`). `overviewData`'s cursor translation, `moveCursor`/`fleetOrder`, and `jumpWaiting` all key on the same `len(m.slots)==0` classic-vs-fleet split — keep them in sync or nav and render diverge. Fleet state is **workspace-mode-only**: `ensureFleetLoaded` no-ops without slots, `enterGlobalMode` resets `fleetEngaged`/`fleetLoading`/`fleetLoadErrors`, and `handleWorkspaceActivated` discards loads that outlive the mode (releasing preview PTYs via `Instance.ReleasePreview` — same for duplicates). A stale overview cursor (killed instance, collapsed group) is healed by `normalizeOverviewCursor` at render (`overviewData`) and nav (`moveCursor`) — don't add per-event cursor bookkeeping.
- **Fleet loading is off the Update goroutine.** `loadWorkspaceForFleet` runs inside a `tea.Cmd` (concurrent with `Update`/`View`, and two run in parallel under `tea.Batch`); it touches only per-workspace locals (fresh storage/instances over a distinct `ConfigDir`), never `home` state — the only model mutation (append slot) happens in the `handleWorkspaceActivated` message handler on the main goroutine. New per-workspace load work follows this split; never build `ui.*` components or touch `m.slots` from the Cmd.
