...
func startEv(id, agentType string) hooks.Event {
	return hooks.Event{Name: hooks.EventSubagentStart, AgentID: id, AgentType: agentType, TranscriptPath: transcript}
}
func stopEv(id string) hooks.Event { return hooks.Event{Name: hooks.EventSubagentStop, AgentID: id} }
func idleEv(name string) hooks.Event {
	return hooks.Event{Name: hooks.EventTeammateIdle, TeammateName: name}
}
func sessionEnd() hooks.Event { return hooks.Event{Name: hooks.EventSessionEnd} }
func parentStop(ts ...hooks.Task) hooks.Event {
	return hooks.Event{Name: hooks.EventStop, Tasks: ts, HasTasks: true}
}
func runningSub(id string) hooks.Task { return hooks.Task{ID: id, Type: "subagent", Status: "running"} }
func runningMate() hooks.Task         { return hooks.Task{ID: "tk", Type: "teammate", Status: "running"} }
...
func TestTracker_PlainLifecycle(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("a1", "Explore")}, map[string]Meta{"a1": plainMeta("Explore", "map code")})
	assert.Equal(t, []View{{Name: "Explore", Description: "map code"}}, tr.Visible())

	tr.Apply([]hooks.Event{stopEv("a1")}, nil)
...
func TestTracker_TeammateCycle(t *testing.T) {
	...
	tr.Apply([]hooks.Event{startEv("amate-1", "mate")}, meta)
	assert.Equal(t, []View{{Name: "mate", Description: "implement task"}}, tr.Visible())

	tr.Apply([]hooks.Event{stopEv("amate-1")}, nil)
	...
	tr.Apply([]hooks.Event{idleEv("mate")}, nil)
	assert.True(t, tr.Visible()[0].Idle)

	tr.Apply([]hooks.Event{startEv("amate-1", "mate")}, nil)
	require.Len(t, tr.Visible(), 1, "re-tasking reuses the agent ID")
	assert.False(t, tr.Visible()[0].Idle)

	tr.Apply([]hooks.Event{stopEv("amate-1"), idleEv("mate")}, nil)
	assert.True(t, tr.Visible()[0].Idle)
}

func TestTracker_FullShutdownRemovesTeammates(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("amate-1", "mate"), stopEv("amate-1"), idleEv("mate")},
	...
	tr.Apply([]hooks.Event{startEv("amate-1", "mate"), stopEv("amate-1"), parentStop()}, nil)
...
func TestTracker_PartialShutdownRemovesStoppingFirst(t *testing.T) {
	...
	tr.Apply([]hooks.Event{startEv("aa-1", "a"), stopEv("aa-1"), parentStop(runningMate())}, nil)
...
func TestTracker_PartialShutdownFallsBackToMostRecentlyStopped(t *testing.T) {
	...
	tr.Apply([]hooks.Event{parentStop(runningMate())}, nil)
...
func TestTracker_SurvivingStoppingTeammateBecomesIdle(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("aa-1", "a"), stopEv("aa-1")}, map[string]Meta{"aa-1": mateMeta("a", "x")})
	tr.Apply([]hooks.Event{parentStop(runningMate())}, nil)
...
func TestTracker_UnknownStopIgnored(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{stopEv("a2e1f32d0ce479a2f")}, nil)
	assert.Empty(t, tr.Visible())
...
func TestTracker_HiddenUntilMeta(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("a1", "Explore")}, nil)
	assert.Empty(t, tr.Visible())
...
func TestTracker_PlainStopBeforeMetaEndsWhenMetaArrives(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("a1", "Explore"), stopEv("a1")}, nil)
...
func TestTracker_TeammateStopBeforeMetaStaysIdle(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("amate-1", "mate"), stopEv("amate-1")}, nil)
...
func TestTracker_TeammateIdleBeforeMetaMarksTeammate(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("amate-1", "mate"), stopEv("amate-1"), idleEv("mate")}, nil)
...
func TestTracker_ParentStopRepairsOutOfOrderEvents(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{stopEv("a1"), startEv("a1", "Explore")}, map[string]Meta{"a1": plainMeta("Explore", "d")})
	require.Len(t, tr.Visible(), 1, "stop processed first, so the agent looks stuck")

	tr.Apply([]hooks.Event{parentStop()}, nil)
	assert.Empty(t, tr.Visible())
}
...
func TestTracker_ParentStopKeepsListedSubagents(t *testing.T) {
	...
	tr.Apply([]hooks.Event{startEv("a1", "Explore"), startEv("a2", "Plan")}, meta)

	tr.Apply([]hooks.Event{parentStop(runningSub("a1"), hooks.Task{ID: "a2", Type: "subagent", Status: "completed"})}, nil)
...
func TestTracker_UnusableTaskListDoesNotReconcile(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("a1", "Explore")}, map[string]Meta{"a1": plainMeta("Explore", "d")})
	tr.Apply([]hooks.Event{{Name: hooks.EventStop}}, nil) // HasTasks false
	assert.Len(t, tr.Visible(), 1)
}
...
func TestTracker_SubagentStopListIsNotUsedForReconciliation(t *testing.T) {
	...
	stop := stopEv("a1")
	stop.Tasks, stop.HasTasks = []hooks.Task{}, true
	tr.Apply([]hooks.Event{stop}, nil)
	assert.Equal(t, []View{{Name: "Plan", Description: "e"}}, tr.Visible())
}

func TestTracker_SessionEndClears(t *testing.T) {
	tr := NewTracker()
	tr.Apply([]hooks.Event{startEv("a1", "Explore"), sessionEnd()}, map[string]Meta{"a1": plainMeta("Explore", "d")})
	assert.Empty(t, tr.Visible())
}
