...
type AppModel struct {
	// title is the owning session's title — every async msg carries it
	// so stale deliveries can be dropped by the app layer.
	title string
...
// NewDocPane creates a single-document review pane rooted at root.
// filePath should be absolute (or relative to the process cwd).
func NewDocPane(title, root, filePath string) *Pane {
	tab := FileTab{
		path:       filePath,
		display:    filePath,
		cursorLine: 1,
...
// NewCodePane creates a multi-file code review pane. Paths in files are
// repo-relative; they are joined onto root so all document I/O is
// absolute while the tab bar keeps the relative label.
func NewCodePane(title, root string, files []gitpkg.FileChange, ref string) *Pane {
...
// persistAllCmd batches a write for every tab that has review state.
func (m *AppModel) persistAllCmd() tea.Cmd {
	var cmds []tea.Cmd
	for i := range m.tabs {
		if m.tabs[i].state != nil {
			cmds = append(cmds, m.persistCmd(m.tabs[i].state))
		}
	}
	if len(cmds) == 0 {
		return nil
...
func (m *AppModel) update(msg tea.Msg) tea.Cmd {
	...
	case LoadedMsg:
		...
		// Install the loaded documents and review state into the tabs.
		for i := range m.tabs {
			if i >= len(msg.Docs) {
				break
			}
...
func (m *AppModel) handleKeyPress(msg tea.KeyPressMsg) tea.Cmd {
	...
	case key.Matches(msg, keys.Quit):
		// Persist every tab, then hand the exit back to the app layer.
		cmd := m.persistAllCmd()
...
func (m *AppModel) modalSubmit() tea.Cmd {
	t := m.tab()
	...
	m.rebuildContent()
	m.updateCommentSidebar()
	return m.persistCmd(t.state)
}

func (m *AppModel) modalDelete() tea.Cmd {
	t := m.tab()
...
func (m *AppModel) handleTextModal(msg tea.KeyPressMsg) tea.Cmd {
...
func (m *AppModel) handleTabSearch(msg tea.KeyPressMsg) tea.Cmd {
	switch msg.String() {
	case "esc":
...
func (m *AppModel) updateTabSearchMatches() {
	...
	for i, t := range m.tabs {
		if strings.Contains(strings.ToLower(t.display), query) {
...
func (m *AppModel) recalculateLayout() {
	...
	mainHeight := m.height - headerHeight - tabBarHeight - footerHeight - frameBorderHeight
...
func (m *AppModel) view() string {
	if m.err != nil {
		return fmt.Sprintf("Error: %v\n\nPress q to leave the review.", m.err)
	}
